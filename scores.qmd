...
```{ojs}
//| output: false
tag_vars = ["dataset", "team", "task_label", "language", "adaptive", "score_type", "scoring_model"]
...
prep_scores = (sc) => {
  return sc.map(obj => {
    return tag_vars.reduce((acc, key) => ({
      ...
      team: obj.team ? obj.team.replaceAll("_", "-") : obj.site.replaceAll("_", "-"),
      dataset: obj.dataset.replaceAll("_", "-")
    })
  })
    .filter(d => d.task_label !== "NA")
    .sort((a, b) => {
...
ti = ({
  ...
  "team":          { label: "Team",          filter: true,  color: true,  tip: true  },
...
::: {.callout-note collapse="true"}
## {{< bi geo class=header-icon >}}Team

```{ojs filter-team}
viewof team_vals = cat_checkbox(tags, "team")
```
:::
...
viewof color_var = Inputs.select(cat_vars, {value: "dataset", label: "Color"})
```
:::
...
allowedDatasetNames = (ds) => {
  ...
  //const extraDatasets = ["levante-data-latest", "levante-data-pilots-hackathon"]
  const defaultDataset = "levante-data-pilots"
  const datasets = ds.filter(d => accessLevels.includes(d.properties.accessLevel))
    .filter(d => d.properties.tableCount > 0)
    .map(d => d.name)
    .filter(n => !n.match("-raw|-metadata-|-example"))
    //.filter(n => n.match("processed") || extraDatasets.includes(n))
    .sort()
