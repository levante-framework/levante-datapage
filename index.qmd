...
"same-different-selection": "Same & Different",
             "memory-game": "Memory",
               "egma-math": "Math",
...
ov = overview.map(obj => ({...obj, task_label: task_info[obj.task_id]})).filter(d => ds.includes(d.ref))
```
...
displayConfig = ([
  { index: 0, property: "dataset",      label: "Datasets",     icon: "archive"        },
  { index: 1, property: "team",         label: "Teams",        icon: "geo"            },
...
```{ojs}
y_var_opts = new Map([["Team", "team"], ["Dataset", "dataset"], ["Language", "language"], ["Task", "task_label"]])
viewof y_var = Inputs.select(y_var_opts, { label: "For each", value: "task_label" })
```

```{ojs}
color_opts = new Map([["", null], ["Team", "team"], ["Dataset", "dataset"], ["Language", "language"], ["Task", "task_label"]])
...
```{ojs}
stack_var_opts = new Map([["", null], ["Team", "team"], ["Dataset", "dataset"], ["Language", "language"], ["Task", "task_label"]])
...
viewof ds = yamultiselect(new Map(dnf.map(d => [d.name.replace("-raw", ""), d.ref])), { value: dnf.map(d => d.ref), placeholder: "Select...", width: "100%" })
```
...
allowedDatasetNames = (ds) => {
  ...
  return ds.filter(d => accessLevels.includes(d.properties.accessLevel))
    ...
    .filter(d => d.ref !== "48qk") // levante_data_pilots_raw
}
```
