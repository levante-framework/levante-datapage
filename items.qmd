...
items_coded = items.map(d => ({...d, lang_group_label: `${d.item_group_label} | ${d.language}` }))
...
item_vars = ["item_group", "item_group_label", "item_uid", "n_responses", "difficulty"]
...
groups = d3.groupSort(l0_items, g => d3.median(g, d => d.difficulty), d => d.item_group_label)
// sort order of language + item group -- above combined
indeces = d3.groups(task_items, d => d.lang_group_label)
            .map(([lang_group_label, vals]) =>
                    [lang_group_label, langs.indexOf(vals[0].language), groups.indexOf(vals[0].item_group_label)])
...
item_dots = (x_var) => {
  return Plot.marks(
    ...
    x: x_var,
    fy: by_lang ? "lang_group_label" : "item_group_label",
    fill: groups.length > 1 ? "item_group_label" : "language",
    r: "n_responses",
...
html`<div class="source"><a href="${model.file_link}" target="_blank">Source</a>: ${model.itemtype} IRT model (${model.invariance_label}) fit to ${model.n_runs} runs, uploaded on ${model.added_at} to model registry version ${model.registry_version}.</div>`
```
